...
public class Main {
    ...
    private static void addFurniture(Build building, int numberRoom) throws Exception {
        ...
        while (true) {
            ...
            idQuery = Integer.parseInt(reader.nextLine());
            if (idQuery == 1) {
                System.out.println("Если хотите добавить диван нажмите - 1");
                idAddFur = Integer.parseInt(reader.nextLine());
                switch (idAddFur) {
                    case 1:
                        System.out.println("Введите название дивана:");
                        nameFurniture = reader.nextLine();
                        System.out.println("Введите площадь дивана");
                        s = Double.parseDouble(reader.nextLine());
                ...
                if (sRoom * 0.7 <= sumS) {
                    ...
                    try {
                        throw new LimitAreaException();
                    } catch (LimitAreaException e) {
                        e.getMessage();
                    }
                }
            } else if (idQuery == 2) {
        ...
        while (true) {
            ...
            if (g == 1) {
                ...
                if (sumWatt * 70  / sRoom + windowsLight >= 4000) {
                    ...
                    try {
                        throw new LimitIlluminationException();
                    } catch (LimitIlluminationException e) {
                        e.getMessage();
                    }
                    return;
                }
